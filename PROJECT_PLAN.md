# Flipkart GRID — Round 2 Prototype
## Theme 2: Event-Driven Congestion (Planned & UNPLANNED)

**Team:** 2 people — Person A (ML/Backend), Person B (Frontend, Next.js)
**Partner tech:** MapmyIndia / Mappls (use their map SDK — judges want to see it used)
...
### Frontend (Person B — Next.js + Mappls)
...
Each endpoint has its own ready-made sample file. Person B builds the UI against these mocks
on Day 1 — no waiting for the real models.
...
```
ml/  (Python FastAPI, port 8000)         ← Person A
...
frontend/  (Next.js, port 3000)          ← Person B
...
### Gateway routes (Person B) — one per endpoint
...
- Person A ships `sample_response.json` DAY 1.
- Person B builds the ENTIRE UI against that mock (live feed, slider, routes) — no waiting.
